<div class="compare__item">
	<div class="compare__item-img">
		<div class="jsTwenty">
			<img src="{$_modx->runSnippet('!pThumb', ['input' => $_pls['photo_before_img'], 'options' => '&w=440&h=400&zc=1'])}" width="440" height="400" alt="">
			<img src="{$_modx->runSnippet('!pThumb', ['input' => $_pls['photo_after_img'], 'options' => '&w=440&h=400&zc=1'])}" width="440" height="400" alt="">
		</div>
	</div>
	<div class="compare__item-content">
		<div class="compare__item-head">
			<h3 class="small-heading">{$pagetitle}</h3>
			{if $_pls['photo_icon']}
			<div class="compare__item-icon">
				<img src="{$_pls['photo_icon']}" alt="">
			</div>
			{/if}
		</div>
		{if $_pls['photo_params']}
		{set $photo_params = json_decode($id |resource: 'photo_params', true)}
		<div class="compare__item-body">
			<ul class="compare-list">
				{foreach $photo_params as $row}
				<li class="compare-list__item">{$row.photo_params_val}</li>
				{/foreach}
			</ul>
		</div>
		{/if}
	</div>
</div>
